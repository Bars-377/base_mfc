...
def index():
    ...
    selected_column = request.args.get('column', None)  # Получаем параметр столбца из URL (если есть)
    ...
    # Начинаем запрос к модели Service
    query = Service.query
    ...
    # Фильтрация по ключевому слову и выбранному столбцу
    if keyword:
        if selected_column and hasattr(Service, selected_column):
            ...
            # Если selected_column пустой или не существует, ищем по всем столбцам
